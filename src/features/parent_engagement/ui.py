...
def _show_new_request_form(parent_id: int, user: dict):
    ...
    query = f"SELECT s.student_id, u.name FROM Students s JOIN Users u ON s.user_id = u.user_id WHERE s.student_id IN ({placeholders})"
    ...
    with st.form("new_request_form", clear_on_submit=True):
        ...
        if not teachers:
            ...
            return
        teacher_options = {f"{t["teacher_name"]} ({t["course_name"]})": t["teacher_id"] for t in teachers}
        selected_teacher_display = st.selectbox("Teacher", list(teacher_options.keys()))
        selected_teacher_id = teacher_options[selected_teacher_display]
        request_type = st.radio("Type", ["Message", "Meeting"], horizontal=True).lower()
        ...
        if request_type == "meeting":
            preferred_times = st.text_area("Preferred Times", max_chars=500, height=100)
        submitted = st.form_submit_button("Submit", type="primary")
        if submitted:
            try:
                result = engagement_service.create_request(parent_id=parent_id, teacher_id=selected_teacher_id, student_id=selected_student_id, request_type=request_type, subject=subject, message=message, preferred_times=preferred_times)
                if result["success"]:
...
def show_parent_requests_page():
    ...
    filtered = requests if filt == "All" else [r for r in requests if r["status"] == filt.lower()]
    if not filtered:
...
def _show_request_card(req: dict, teacher_id: int):
    is_pend = req["status"] == "pending"
    with st.expander(f"{req["request_type"].title()} from {req["parent_name"]} - {req["subject"]}", expanded=is_pend):
        c1, c2 = st.columns(2)
        with c1:
        ...
        with st.form(f"form_{req["request_id"]}"):
            resp = st.text_area("Response", max_chars=2000, height=120, key=f"r_{req["request_id"]}")
            c1, c2, c3 = st.columns(3)
            with c1:
                send = st.form_submit_button("Send", type="primary")
            with c2:
                approve = st.form_submit_button("Approve", disabled=not is_pend)
            with c3:
                reject = st.form_submit_button("Reject", disabled=not is_pend)
            if send and resp:
                result = engagement_service.respond_to_request(req["request_id"], teacher_id, resp)
                if result["success"]:
                    st.success("Sent!")
                    st.rerun()
            elif approve and resp:
                result = engagement_service.respond_to_request(req["request_id"], teacher_id, resp, "approved")
                if result["success"]:
                    st.success("Approved!")
                    st.rerun()
            elif reject and resp:
                result = engagement_service.respond_to_request(req["request_id"], teacher_id, resp, "rejected")
                if result["success"]:
                    st.success("Rejected!")
                    st.rerun()
            elif (send or approve or reject) and not resp:
                st.warning("Please enter response.")
