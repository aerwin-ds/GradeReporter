...
def get_pages_for_role(role: str) -> List[Dict[str, str]]:
    ...
    if role == ROLES['STUDENT']:
        ...
        if FEATURES['announcements']:
            pages.append({"name": "Announcements", "icon": "📢", "module": "announcements"})
        if FEATURES['after_hours']:
            pages.append({"name": "Ask a Question", "icon": "❓", "module": "after_hours"})

    elif role == ROLES['PARENT']:
        pages.extend([
            {"name": "My Children", "icon": "👨‍👩‍👧‍👦", "module": "parent_dashboard"},
        ])
        if FEATURES['parent_engagement']:
            pages.append({"name": "Contact Teachers", "icon": "✉️", "module": "parent_engagement"})
        if FEATURES['announcements']:
            pages.append({"name": "Announcements", "icon": "📢", "module": "announcements"})

    elif role == ROLES['TEACHER']:
        pages.extend([
            {"name": "My Courses", "icon": "📚", "module": "teacher_dashboard"},
        ])
        if FEATURES['announcements']:
            pages.append({"name": "Announcements", "icon": "📢", "module": "announcements"})
        if FEATURES['parent_engagement']:
            pages.append({"name": "Parent Requests", "icon": "📬", "module": "parent_engagement"})
        if FEATURES['after_hours']:
            pages.append({"name": "Student Questions", "icon": "❓", "module": "after_hours"})

    elif role == ROLES['ADMIN']:
        pages.extend([
            {"name": "Admin Dashboard", "icon": "⚙️", "module": "admin_dashboard"},
        ])
        if FEATURES['announcements']:
            pages.append({"name": "Announcements", "icon": "📢", "module": "announcements"})
